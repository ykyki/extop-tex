...
\begin{proof}
	...
	$ X $の有限部分集合$ A_y $が存在して$ \setcomp{V_{(x,y)}}{x \in A_y} $が$ X $の開被覆となる. 
...
\end{definition}
P-spaceは\cite{GillmanHenriksen1954}で離散空間よりも広いクラスとして導入され, 
"pseudo discrete"のPに由来している.

\begin{proposition}[{\cite[Theorem~5.3]{GillmanHenriksen1954}}]
	位相空間$ X $において次は同値である.
	\begin{enumerate}
		\item[1] $ X $がP-空間である.
		\item[2] $ X $の任意の \Gdelta 集合が開集合である.
		\item[3] 位相空間$ X $に対して, $ C(X) $を$ X $上の実数値連続関数全体からなる環とする. 
...
\begin{proposition}[{\cite[Corollary~5.6, 5.7, 5.8]{GillmanHenriksen1954}}]
	\label{prop: Subspace of a P-space is also a P-space}
	P空間の部分空間(非交和, 有限個の積)もまたP空間である.
	\qed
\end{proposition}

\begin{proposition}[{\cite[Theorem~2.1]{Arvind1972}}]
...
\end{proposition}
この命題の証明の前に次の補題を示す. 本命題の証明は次の補題の証明の後に示す.
...
\begin{definition}
	位相空間$ X $の被覆$ \mathscr{U} $が\indexjj{きやく}{既約}{irreducible}であるとは, 
...
\begin{proof}
	...
	$ (F_n)_{n \in \N} $が単調増加であることから$ y \in \mathscr{W}_i $となる
