...
\begin{proposition}
	\topT{4} 空間$ X $と部分集合$ A $に対して, $ A $の元を同一視した商空間を$ X / A $とする.
...
\begin{proposition}
	...
	$ f^{-1}[\mathscr{U}] $は$ X $で局所有限である.
\end{proposition}

\begin{proof}
	任意の点$ x \in X $について$ f(x) $の開近傍$ V $が存在して
	$ \setcomp{U \in \mathscr{U}}{U \cap V \neq \emptyset} $は有限である.
	$ f $は連続なので$ x $の開近傍$ W $が存在して$ f[W] \subset V $が成り立つ.
	よって, $ \setcomp{U \in \mathscr{U}}{U \cap f[W] \neq \emptyset} $は有限なので
	$ \setcomp{U' \in f^{-1}[\mathscr{U}]}{U' \cap W \neq \emptyset} $も有限である.
\end{proof}
...
\begin{proof}
	...
	$ f^{-1}(y) $がコンパクトなので, $ \mathscr{U} $の有限部分集合$ \mathscr{U}_y $が存在して
	$ f^{-1}(y) \subset \bigcup \mathscr{U}_y $となる.
	$ f $が閉写像であることから$ y $の開近傍$ U_y $が存在して
	$ f^{-1}(y) \subset f^{-1}[U_y] \subset \bigcup \mathscr{U}_y $が成り立つ.
	$ \setfamily{U_y}{y \in Y} $はパラコンパクト空間$ Y $の開被覆なので
	局所有限な$ Y $の開被覆$ \mathscr{V} $であって
	$ \mathscr{V} < \setfamily{U_y}{y \in Y} $を満たすものが存在する.
	このとき, $ f^{-1}[\mathscr{V}] $は$ X $の開被覆であり, \cref{p00001}より
	局所有限でもある. また, 任意の$ V \in \mathscr{V} $に対して
	$ y(V) \in Y $であって$ f^{-1}[V] \subset f^{-1}[U_{y(V)}] \subset \bigcup \mathscr{U}_{y(V)} $を
	満たすものが存在する.
	ここで, $ \mathscr{W} \defeq \setcomp{f^{-1}[V] \cap U}{V \in \mathscr{V}, U \in \mathscr{U}_{y(V)}} $と定める.
	$ f^{-1}[V] $が開被覆であり,
	任意の$ V \in \mathscr{V} $に対して$ f^{-1}[V] \subset \bigcup \mathscr{U}_{y(V)} $なので
	$ \mathscr{W} $は$ X $の開被覆である.
	また, $ \mathscr{W} $の定義から明らかに$ \mathscr{W} < \mathscr{U} $である.
	さらに, $ f^{-1}[\mathscr{V}] $が局所有限であることと, 各$ V \in \mathscr{V} $について
	$ \mathscr{U}_{y(V)} $が有限で, $ f^{-1} [V] \subset \bigcup \mathscr{U}_{y(V)}$を満たすことから
	$ \mathscr{W} $が局所有限であることがわかる.
\end{proof}
