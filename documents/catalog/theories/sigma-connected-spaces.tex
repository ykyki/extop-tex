...
\begin{proof}
	...
	$\bigcap \mathscr{K} = Q = U_1 \cup U_2 $に補題\locref{lemma:1}を適用して,
...
\begin{proof}
	...
	補題\locref{lemma:2}より$ C = \bigcap \mathscr{K} $である. もし仮に$ C \cap \topbry A = \emptyset $であったとする.
...
\begin{proof}
	...
	補題\locref{lemma:3}より, $ C \cap \topbry\topcl V \neq \emptyset $なので, この集合の元$ y $が存在する.
...
\begin{proof}[定理\ref{thm:Sierpinski_continuum}]
	...
	補題\locref{lemma:4}よりある連続体$ C_0 \subset X $が存在して$ C_0 \cap  F_0 = \emptyset$となり,
	さらに$ C \cap F_1, C \cap F_2, \ldots $の中でも少なくとも2つは非空となる.
	補題\locref{lemma:4}を再び$ C_0 $と閉被覆$ \setfamily{C_0 \cap F_i}{i \in \N_{>0}} $について適用する.
