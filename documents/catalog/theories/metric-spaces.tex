...
\begin{proof}
	...
	$ B(x, \varepsilon) $とすると,
	$ B(x, \varepsilon) = \prod_{i=0}^n B_i(x_i, \varepsilon) \subset U $
	が成り立つことを確かめられる.
	よって, $ U \in \mathcal{O}_{d^{(\infty)}} $が成り立つ.

	次に$ V \in \mathcal{O}_{d^{(\infty)}} $と任意の点$ x \in V $をとる.
	ある$ \delta > 0 $が存在して$ B(x, \delta) \subset V $が成り立ち,
	$ B(x, \delta) = \prod_{i=0}^n B_i(x_i, \delta) $に注意すれば
...
\begin{proof}
	...
	B_n(x_n, \varepsilon) $が成り立ち, $ p_n $は連続である.
